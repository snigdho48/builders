...
import {
  createLandShareListing,
  deleteLandShareListing,
...
export function AdminLandShareListingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [{ items }, agentList] = await Promise.all([
        getLandShareListingsPaged({ pageSize: 200, includeInactive: true }),
        listAgents(token),
      ])
      setRows(items)
      setAgents(agentList)
  ...
  async function remove(p: LandShareListing) {
    if (!window.confirm(`Delete “${p.title}”?`)) return
    const token = localStorage.getItem("accessToken")
    if (!token) return
    try {
      await deleteLandShareListing(p.id, token)
  ...
  const filteredRows = useMemo(() => {
    ...
    return list
  }, [rows, tableSearch, tableActive, tableAgentId])
