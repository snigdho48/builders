...
from apps.models import LandShareListing
from apps.permissions import CanAccessLandShareListings
from apps.serializers.land_share_listings import LandShareListingSerializer
...
class LandShareListingViewSet(StandardModelViewSet):
    serializer_class = LandShareListingSerializer
    queryset = (
        LandShareListing.objects.select_related("assigned_agent", "assigned_agent__profile")
        .all()
        .order_by("-created_at")
    )
    ...
    def get_permissions(self):
        if self.action in ["list", "retrieve"]:
            return [CanAccessLandShareListings()]
        return [permissions.IsAuthenticated(), CanAccessLandShareListings()]
    ...
    def _queryset_for_request_role(self, queryset, user, params):
        ...
        if params.get("managed_by_me") == "1":
            if not user.is_authenticated:
                return LandShareListing.objects.none()
